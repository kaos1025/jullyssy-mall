...
// 공개 탑배너 fetcher(getActiveTopBanners) 단위 테스트 — (shop)/layout의 dynamic 원인 제거 회귀 방지.
...
import { getActiveTopBanners } from "./banners"
...
describe("getActiveTopBanners (캐시 격리 공개 fetcher)", () => {
  it("admin client + is_active/기간 필터 + sort_order 정렬, 데이터 반환", async () => {
    setResponse({ data: [{ id: "b1" }, { id: "b2" }], error: null })

    const result = await getActiveTopBanners()

    expect(result.map((b) => b.id)).toEqual(["b1", "b2"])
    expect(createServerClient).not.toHaveBeenCalled()
    expect(createAdminClient).toHaveBeenCalledTimes(1)
    expect(builders).toHaveLength(1)
    expect(builders[0].table).toBe("top_banners")
    ...
    expect(callsOf(builders[0], "order")).toEqual([["sort_order", { ascending: true }]])
  })
  ...
  it("쿼리 에러 → [] (throw 안 함, 레이아웃 graceful degrade)", async () => {
    ...
    await expect(getActiveTopBanners()).resolves.toEqual([])
