...
const handler = async (request: NextRequest) => {
  ...
  const processOne = async (order: PollTarget): Promise<PollOutcome> => {
    if (!order.courier || !order.tracking_no) return "skipped"
    try {
      const result = await tracker.fetchStatus(order.courier, order.tracking_no)
      if (!result.delivered) return "skipped"
      ...
      if (mark.status === "transitioned") return "transitioned"
      ...
      if (err instanceof UnsupportedCourierError) return "unsupported"
